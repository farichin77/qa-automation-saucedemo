...
public class CheckoutTest extends BaseTest {
    ...
    public void testSuccessfulCheckout() {
        // Verify initial checkout page state
        Assert.assertTrue(driver.getCurrentUrl().contains("checkout-step-one.html"), "Should be on checkout step one");
        
        // Fill checkout info and verify
        checkoutPage.fillCheckoutInfo("John", "Doe", "12345");
        Assert.assertTrue(driver.getCurrentUrl().contains("checkout-step-two.html"), "Should proceed to checkout step two");
        ...
        Assert.assertTrue(driver.getCurrentUrl().contains("checkout-complete.html"), "Should be on complete page");
        Assert.assertTrue(driver.findElement(By.className("complete-header")).isDisplayed(), "Completion header should be visible");
    }
    ...
    public void testEmptyFirstName() {
        ...
        Assert.assertTrue(driver.getCurrentUrl().contains("checkout-step-one.html"), "Should stay on checkout step one");
        Assert.assertTrue(driver.findElement(By.cssSelector("[data-test='error']")).isDisplayed(), "Error message should be visible");
    }
    ...
    public void testEmptyLastName() {
        ...
        Assert.assertTrue(driver.getCurrentUrl().contains("checkout-step-one.html"), "Should stay on checkout step one");
        Assert.assertEquals(driver.findElement(By.id("first-name")).getAttribute("value"), "John", "First name should be preserved");
    }
    ...
    public void testEmptyPostalCode() {
        ...
        Assert.assertTrue(driver.getCurrentUrl().contains("checkout-step-one.html"), "Should stay on checkout step one");
        Assert.assertEquals(driver.findElement(By.id("first-name")).getAttribute("value"), "John", "First name should be preserved");
        Assert.assertEquals(driver.findElement(By.id("last-name")).getAttribute("value"), "Doe", "Last name should be preserved");
    }
